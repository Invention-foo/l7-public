...
export async function POST(request: Request) {
  try {
    ...
    const expectedMessage = `Add exception for user ${userId} at timestamp ${timestamp}`;
    ...
    const { data: existingException } = await supabaseAdmin
    ...
    if (existingException) {
      return NextResponse.json({ 
        success: false, 
        message: "Exception already exists" 
      }, { status: 409 });
    }
